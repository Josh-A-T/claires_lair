...
class Album {
  static async create(albumData) {
    const { title, artist_id, release_year, label } = albumData;
    const result = await pool.query(
      `INSERT INTO albums (title, artist_id, release_year, label) 
       VALUES ($1, $2, $3, $4) 
       RETURNING *`,
      [title, artist_id, release_year, label]
    );
  ...
  static async update(id, albumData) {
    const { title, release_year, label } = albumData;
    const result = await pool.query(
      `UPDATE albums 
       SET title = $1, release_year = $2, label = $3, updated_at = CURRENT_TIMESTAMP
       WHERE id = $4
       RETURNING *`,
      [title, release_year, label, id]
    );
    return result.rows[0];
  }
